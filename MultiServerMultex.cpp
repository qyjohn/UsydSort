...
struct sort_thread_args
{
	...
	int mode, socket, server_thread_id, sender_id;
	char* temp_dir;
	SortRecordQueue *sr_queue;
};
...
int main(int argc, char* argv[])
{
	...
	unsigned cpu_cores = std::thread::hardware_concurrency() - reserved_cpu_core;
...
void *server_thread(void *args)
{
	...
	while (noThread < this_total_clients)
	{
		...
		sortArgs[noThread].sr_queue = &sr_queue;
		if (this_mode == 1)
		{
	...
	if (this_mode == 0)
	{
		...
		if (io_mode == 0)
		{
			save_queue_to_file_direct_io(&sr_queue.q, this_output_filename, this_thread_id);
		}
		else
		{
			save_queue_to_file_buffer_io(&sr_queue.q, this_output_filename, this_thread_id);	
		}
...
void *sort_thread (void *args)
{
	...
	SortRecordQueue *sr_queue = myArgs->sr_queue;
