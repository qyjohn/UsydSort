...
void *process_buffer_data_thread(void *args);
...
struct receive_data_thread_args
{
	// mode = 0 (default mode)
	//
	...
	std::vector<SortRecord> *my_vector;
};

struct process_buffer_data_thread_args
{
...
int main(int argc, char* argv[])
{
	...
	for (i=1; i<batch; i++)
	{
		process_buffer_data_thread_args pbdt_args[mode_mark];
		pthread_t all_process_buffer_data_threads[mode_mark];
		for (j=0; j<mode_mark; j++)
		{
			// Create a server thread
			int server_thread_id = i*mode_mark + j;
			pbdt_args[j].server_thread_id = server_thread_id;
			pbdt_args[j].temp_dir = args[server_thread_id].temp_dir;
			pbdt_args[j].output_filename = args[server_thread_id].output_filename;

			pthread_create(&all_process_buffer_data_threads[j], NULL, process_buffer_data_thread, (void*) &pbdt_args[j]); 
		}

		// Wait for all the server threads to exit. 
		for (j=0; j<mode_mark; j++)
		{
			pthread_join(all_process_buffer_data_threads[j], NULL);
		}
	}
}
...
void *server_thread(void *args)
{
	...
	std::vector<SortRecord> vectors[this_total_clients];

	int clientSocket[this_total_clients];
	struct receive_data_thread_args	sortArgs[this_total_clients];
	...
	while (noThread < this_total_clients)
	{
		...
		sortArgs[noThread].my_vector = &vectors[noThread];	
		...
		pthread_create(&threadA[noThread], NULL, receive_data_thread, (void*) &sortArgs[noThread]); 
		noThread++;
	}
	...
	if (this_mode == 0)
	{
		...
		log_progress(message);
		std::priority_queue<SortRecord> final_queue;
		for(int i = 0; i < this_total_clients; i++)
		{
			for (int j=0; j<vectors[i].size(); j++)
			{
				final_queue.push(vectors[i].at(j));
			}
			vectors[i].clear();
		}			
/*
...
*/
		...
		log_progress(message);
		// BufferIO seems to be better in performance
//		save_queue_to_file_buffer_io(&queues[0], this_output_filename, this_thread_id);	
		save_queue_to_file_buffer_io(&final_queue, this_output_filename, this_thread_id);	
...
/*	
	...
	{
		sprintf(message, "Thread %04d: Merging intermediate results.", this_thread_id);
		log_progress(message);
...
void *receive_data_thread (void *args)
{
	struct receive_data_thread_args *myArgs = (struct receive_data_thread_args*) args;
	...
	std::vector<SortRecord> *v = myArgs->my_vector;

	// Print out debug message
	sprintf(message, "Thread %04d: Sender %04d is now connected with mode %d.", server_thread_id, sender_id, mode);
	log_progress(message);
	...
	// Default mode, save incoming data to a vector
	if (mode == 0)
	{
		while(true)
		{
...
//				q->push(sr);
	...
	// Buffer mode, save incoming data to intermediate file
	else if (mode == 1)
	{
		int temp_file_id = 0;
		int temp_rec_count = 0;
		char filename[100];
		sprintf(filename, "%s/%lu", temp_dir, pthread_self());
		std::ofstream outfile (filename,std::ofstream::binary);
		while(true) 
		{
			// Attempt to receive [buffer_size] bytes each time, unless the client disconnects
			size=recv(sock, buffer, buffer_size, MSG_WAITALL);
			temp[0] = buffer[size-4];
			temp[1] = buffer[size-3];
			temp[2] = buffer[size-2];
			temp[3] = buffer[size-1];
...
void *process_buffer_data_thread(void *args)
{
	struct process_buffer_data_thread_args *myArgs = (struct process_buffer_data_thread_args*) args;

	int server_thread_id = myArgs->server_thread_id;
	char* temp_dir = myArgs->temp_dir;
	char* output_filename = myArgs->output_filename;

	merge_temp_files_and_save(temp_dir, output_filename, server_thread_id);
}
